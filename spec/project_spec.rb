...
describe Project do
  ...
  describe '.find' do
    it 'returns a project by a volunteers project_id' do
      project1 = Project.new({:title => 'Teaching Kids to Code', :id => nil})
      project1.save
      project2 = Project.new({:title => 'Teaching Ruby to Kids', :id => nil})
      project2.save
      volunteer = Volunteer.new({:name => "Reid", :id => nil, :project_id => project1.id})
      volunteer.save
      expect(Project.find(volunteer.project_id)).to eq project1
    end
  end
